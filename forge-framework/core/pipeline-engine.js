...
const os = require("os");
const { createContext, expandScope: expandContextScope } = require("./workspace-context");
...
const SCOPE_PROFILES = {
  javascript: {
    scope: ["src/**", "lib/**", "tests/**", "test/**", "__tests__/**", "routes/**", "middleware/**"],
    readonly: ["package-lock.json", "yarn.lock", "pnpm-lock.yaml", "tsconfig.json"],
    ignore: ["node_modules/**", "dist/**", "build/**", ".next/**", "coverage/**"],
    frameworks: {
      "next.config": { scope_add: ["pages/**", "app/**", "components/**", "public/**"] },
      "vite.config": { scope_add: ["public/**"] },
      "angular.json": { scope_add: ["src/app/**", "e2e/**"] },
      "nuxt.config": { scope_add: ["pages/**", "components/**", "composables/**", "server/**"] },
    }
  },
  typescript: { inherits: "javascript" },
  python: {
    scope: ["src/**", "app/**", "tests/**", "test/**"],
    readonly: ["pyproject.toml", "setup.py", "requirements.txt", "poetry.lock"],
    ignore: ["__pycache__/**", ".venv/**", "venv/**", ".tox/**", "dist/**", "build/**"],
    frameworks: {
      "manage.py": { scope_add: ["*/models/**", "*/views/**", "*/templates/**", "*/migrations/**"] },
      "app.py": { scope_add: ["templates/**", "static/**"] },
    }
  },
  go: {
    scope: ["cmd/**", "internal/**", "pkg/**", "**/*_test.go"],
...
function getScopeProfile(language, cwd) {
  let profile = SCOPE_PROFILES[language];
  if (!profile) return { scope: ["**"], readonly: [], ignore: ["node_modules/**", ".git/**"] };

  // Handle inheritance
  if (profile.inherits) {
    profile = { ...SCOPE_PROFILES[profile.inherits], ...profile };
  ...
  if (profile.frameworks && cwd) {
    for (const [marker, config] of Object.entries(profile.frameworks)) {
      try {
        const files = fs.readdirSync(cwd);
        if (files.some(f => f.startsWith(marker))) {
          result.scope.push(...(config.scope_add || []));
        }
      } catch {}
    }
  }
...
function detectStack() {
  ...
  const scopeProfile = getScopeProfile(detected.language, CWD);
  detected.scope_profile = scopeProfile;
  return detected;
}
...
function engineInit(artifactDir, request, type, scale, options) {
  ...
  const pipelineName = (options?.trivial || pl === "trivial") ? "trivial" :
    ...
    (options?.ralph || pl === "ralph") ? "ralph" :
    (options?.analyze || pl === "analyze") ? "analyze" : "standard";
  ...
  if (options?.analyze || pl === "analyze") skippedSteps.push("research", "architect_guide", "plan", "plan_check", "checkpoint", "branch", "execute", "verify");
  ...
  // Create workspace context (v8.0)
  try {
    const detectResult = detectStack();
    const scopeProfile = detectResult.scope_profile || { scope: ["**"], readonly: [], ignore: [] };
    createContext(CWD, scopeProfile, path.relative(CWD, artifactDir));
  } catch (err) {
    // Context creation failure is non-fatal — gate guard falls back to v7 behavior
    process.stderr.write("Warning: workspace context creation failed: " + (err.message || "") + "\n");
  }
...
try {
  ...
  switch (command) {
    ...
    case "engine-expand-scope": {
      requireArgs(args, 2, "engine-expand-scope <dir> <pattern> [reason] [step]");
      const artifactDir = args[0];
      const pattern = args[1];
      const reason = args[2] || "manual expansion";
      const step = args[3] || "unknown";
      try {
        expandContextScope(artifactDir, CWD, pattern, reason, step);
        result = { expanded: true, pattern };
      } catch (err) {
        result = { expanded: false, reason: err.message };
      }
      break;
    }
    case "help":
    default:
      result = {
        commands: [
          ...
          "engine-expand-scope <dir> <pattern> [reason] [step]",
          "help"
        ]
      };
  }
