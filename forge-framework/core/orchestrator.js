...
function readGatekeeperState(sessionId) {
  const stateFile = path.join(STATE_DIR, `gatekeeper-${sessionId || 'default'}.json`);
  try {
    const data = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
    // State is valid if written within last 60 seconds (same turn)
    if (data && (Date.now() - (data.ts || 0)) < 60000) return data;
  } catch {}
  return null;
}
...
function formatPipelineContext(state, sessionId) {
  ...
  // Workspace context scope summary (v8.0)
  try {
    const { loadContext } = require("./workspace-context");
    const ctx = loadContext(state.artifact_dir, CWD);
    if (ctx) {
      lines.push(`Scope: ${(ctx.scope || []).slice(0, 5).join(", ")}${ctx.scope.length > 5 ? " ..." : ""}`);
      if (ctx.readonly && ctx.readonly.length > 0) {
        lines.push(`Readonly: ${ctx.readonly.join(", ")}`);
      }
      if (ctx.scope_expansions && ctx.scope_expansions.length > 0) {
        lines.push(`Expanded: ${ctx.scope_expansions.map(e => e.pattern).join(", ")}`);
      }
    }
  } catch {}
...
function main() {
  try {
    ...
    // 4. Gatekeeper routing injection (only if no active pipeline)
    if (!activePipeline) {
      try {
        const gkState = readGatekeeperState(input.session_id);
        if (gkState && (gkState.category === 'code-modify' || gkState.category === 'skill-task')) {
          const skillName = gkState.skill || 'forge-dev';
          const modeHint = gkState.config?.mode ? ` (mode: ${gkState.config.mode})` : '';
          const typeHint = gkState.config?.type ? `, type: ${gkState.config.type}` : '';
          output += `\n[Gatekeeper] Intent: ${gkState.category} → invoke the ${skillName} skill${modeHint}${typeHint}\n`;
          output += `To proceed, call: Skill("${skillName}")\n`;
        }
        // For discuss/question/command/meta: no injection — Claude responds directly
      } catch {}
    }
