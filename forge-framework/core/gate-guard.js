...
function isProjectFile(filePath) {
  ...
  try {
    ...
    return resolved.startsWith(cwdResolved + path.sep) || resolved === cwdResolved;
  } catch { return true; } // fail-closed: assume it's a project file if can't resolve
}
    ...
    for (const pattern of BASH_WRITE_PATTERNS) {
      ...
      if (match) {
        ...
        if (raw) {
          ...
          // Resolve common shell variables to actual paths
          if (cleaned.includes('$')) {
            try {
              cleaned = cleaned
                .replace(/\$HOME|\$\{HOME\}/g, process.env.HOME || '')
                .replace(/\$PWD|\$\{PWD\}/g, CWD || process.cwd())
                .replace(/\$\w+/g, ''); // unknown vars → empty (fail-open for path check)
            } catch {}
          }
          return cleaned;
        }
...
function main() {
  ...
  try {
    ...
    // Meta-workflow exclusion: check if Gatekeeper classified as non-code-modify
    try {
      const gkFile = path.join(STATE_DIR, `gatekeeper-${input.session_id || ""}.json`);
      if (fs.existsSync(gkFile)) {
        const gkState = JSON.parse(fs.readFileSync(gkFile, "utf8"));
        if (gkState.category && gkState.category !== "code-modify" && gkState.category !== "skill-task") {
          process.exit(0); // Non-code task — gate guard not needed
        }
      }
    } catch {}
    ...
    if (!state) {
      ...
      if (["Edit", "Write", "NotebookEdit"].includes(toolName)) {
        ...
        if (isProjectFile(filePath) && isCodeFile(filePath)) {
          process.stderr.write(
      ...
      if (toolName === "Bash") {
        ...
        if (!SAFE_BASH_COMMANDS.test(cmd)) {
          ...
          if (targetFile && isProjectFile(targetFile) && isCodeFile(targetFile)) {
            process.stderr.write(
  ...
  // Load workspace context (v8.0) — optional, falls back to v7 behavior on failure
  let workspaceContext = null;
  if (state && artifactDir) {
    try {
      workspaceContext = loadContext(state.artifact_dir, CWD);
    } catch {}
  }
  ...
  try {
    if (["Edit", "Write"].includes(toolName)) {
      ...
      if (workspaceContext) {
        const classification = classifyFile(filePath, workspaceContext);
        if (classification === "ignore") {
          // Invisible to pipeline — skip all gate checks for this file
        } else if (classification === "readonly") {
          process.stderr.write(`🚫 GATE BLOCKED: "${path.basename(filePath)}" is in readonly scope.\n`);
          writeAuditLog("Gate 2 Readonly", toolName, filePath, input.session_id);
          process.exit(2);
        } else if (classification === "out-of-scope") {
          // Not our file — skip gate
        } else {
          // "scope" or "external" → apply gate
          shouldCheck = true;
        }
      } else {
        shouldCheck = isProjectFile(filePath) && isCodeFile(filePath);
      }
    ...
    if (toolName === "Bash") {
      ...
      if (!SAFE_BASH_COMMANDS.test(cmd)) {
        ...
        if (targetFile) {
          let shouldCheck = false;
          if (workspaceContext) {
            const classification = classifyFile(targetFile, workspaceContext);
            if (classification === "ignore") {
              // Invisible to pipeline — skip
            } else if (classification === "readonly") {
              process.stderr.write(`🚫 GATE BLOCKED: "${path.basename(targetFile)}" is in readonly scope.\n`);
              writeAuditLog("Gate 2B Readonly", toolName, targetFile, input.session_id);
              process.exit(2);
            } else if (classification === "out-of-scope") {
              // Not our file — skip
            } else {
            ...
            shouldCheck = isProjectFile(targetFile) && isCodeFile(targetFile);
          }
